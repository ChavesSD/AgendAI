...
// Verificar se App já foi declarado para evitar redeclaração
if (typeof window.App === 'undefined') {
    ...
    console.log('App já foi declarado, reutilizando objeto existente');
}

// Adicionar ou atualizar os métodos do App existente
Object.assign(window.App, {
    ...
    router() {
        ...
        // Rota para página de empresas
        if (path === '/admin/companies') {
            if (this.checkAccess('admin')) {
                console.log('Carregando página de empresas...');
            ...
            .then(html => {
                try {
                    ...
                    if (viewContent) {
                        ...
                        // Verificar se é a view admin-companies e adicionar botão Nova Empresa se necessário
                        if (viewName === 'admin-companies') {
                            console.log('📋 View de empresas detectada, verificando botão Nova Empresa...');
                            try {
                                // Verificar se o botão já existe
                                if (!document.getElementById('newCompanyBtn')) {
                                    console.log('🔍 Botão Nova Empresa não encontrado, criando...');
                                    ...
                                    if (header) {
                                        ...
                                        btnNovaEmpresa.onclick = function(e) {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            console.log('🖱️ Botão Nova Empresa clicado! Abrindo modal...');
                                        ...
                                        console.warn('⚠️ Cabeçalho para adicionar botão não encontrado');
                                    }
                                } else {
                                    console.log('✅ Botão Nova Empresa já existe');
                                }
                            } catch (error) {
                                console.error('❌ Erro ao verificar/criar botão Nova Empresa:', error);
                            }
                        }
